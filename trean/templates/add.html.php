...
<tr>
  ...
  <td><input type="text" name="url" size="40" value="<?php echo htmlspecialchars(Util::getFormData('url', 'http://')) ?>" /></td>
</tr>
...
<tr>
  ...
  <td>
   <select name="f">
    <?php echo Trean::folderSelect(Util::getFormData('f'), PERMS_EDIT, true) ?>
   </select>
  </td>
</tr>
...
<tr>
  ...
  <td>
   ...
   <input type="button" class="button" value="<?php echo _("Cancel") ?>" onclick="<?php echo !Util::getFormData('popup') ? 'window.history.go(-1);' : 'window.close();'; ?>" />
  </td>
</tr>
