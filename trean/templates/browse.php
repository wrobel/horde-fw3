...
<?php
$option_move = Trean::countFolders(PERMS_EDIT);
...
$option_edit = (!empty($GLOBALS['folder']) ? $GLOBALS['folder']->hasPermission(Auth::getAuth(), PERMS_EDIT) : false);
$option_delete = (!empty($GLOBALS['folder']) ? $GLOBALS['folder']->hasPermission(Auth::getAuth(), PERMS_DELETE) : false);
...
<?php if (!empty($GLOBALS['folder']) && Auth::getAuth()): ?>
 ...
 <?php if ($GLOBALS['folder']->getParent() && $option_delete): ?>
  <form name="trean_del_folder" action="edit.php" method="get">
   ...
   <input type="hidden" name="f" value="<?php echo $GLOBALS['folder']->getId() ?>" />
   <input type="image" src="<?php echo $GLOBALS['registry']->getImageDir() . '/folders/folder_delete.png' ?>" value="<?php echo _("Delete") ?>" id="sf_del" onclick="confirm_delete(); return false;" />
 ...
 <?php if ($GLOBALS['folder']->get('owner') == Auth::getAuth()): ?>
  <p>
   <?php echo Horde::link(Horde::applicationUrl('perms.php?cid=' . $GLOBALS['folder']->getId()), _("Control access to this folder"), '', '_blank', 'popup(this.href); return false;')
       . Horde::img('perms.png', '', '', $GLOBALS['registry']->getImageDir('horde')) . _("Control access to this folder") . '</a>' ?>
  </p>
 <?php endif; ?>
  </div>
 </div>
...
if (!is_a($folders, 'PEAR_Error')) {
    $params = array('icondir' => $GLOBALS['registry']->getImageDir('horde') . '/tree',
                    'icon' => 'folder.png',
...
?>
 ...
 <div id="bookmarkActions">
  <label for="ba_new">
   <input type="image" src="<?php echo $GLOBALS['registry']->getImageDir() . '/add.png' ?>" name="new_bookmark" value="<?php echo _("New Bookmark") ?>" id="ba_new" />
   <?php echo Horde::highlightAccessKey(_("_New Bookmark"), Horde::getAccessKey(_("_New Bookmark"))) ?>
  </label>

  <label for="ba_edit">
   <input type="image" src="<?php echo $GLOBALS['registry']->getImageDir() . '/edit.png' ?>" name="edit_bookmarks" value="<?php echo _("Edit Bookmarks") ?>" id="ba_edit" />
   <?php echo Horde::highlightAccessKey(_("_Edit Bookmarks"), Horde::getAccessKey(_("_Edit Bookmarks"))) ?>
  </label>
  <?php if ($option_delete): ?>
  <label for="ba_del">
   <input type="image" src="<?php echo $GLOBALS['registry']->getImageDir() . '/delete.png' ?>" name="delete_bookmarks" value="<?php echo _("Delete Bookmark") ?>" id="ba_del" />
