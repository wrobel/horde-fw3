<?php
/**
 * $Horde: trean/lib/Bookmarks.php,v 1.96 2008/04/08 06:47:35 chuck Exp $
 *
 * @package Trean
 */
...
class Trean_Bookmarks {
    ...
    function Trean_Bookmarks()
    {
        ...
        Horde::callHook('_horde_hook_share_init', array($this, 'trean'));
    }
                             ...
                             $sortby = 'title', $sortdir = 0, $from = 0, $count = 0)
    {
        ...
        $folderIds = $this->listFolders(Auth::getAuth(), PERMS_READ);
        ...
        foreach ($search_criteria as $criterion) {
            $clause = Horde_SQL::buildClause($GLOBALS['trean_db'],
                                             ...
                                             $criterion[1],
                                             String::convertCharset($criterion[2],
        ...
        $sql = 'SELECT bookmark_id, folder_id, bookmark_url, bookmark_title, bookmark_description,
                       bookmark_clicks, bookmark_rating, favicon_id, bookmark_http_status
    ...
    {
        $folderIds = $this->listFolders(Auth::getAuth(), PERMS_EDIT);
     ...
     */
    function listFolders($userid, $perm = PERMS_SHOW, $parent = null, $allLevels = true)
    {
        if (is_null($parent)) {
     ...
     */
    function &getFolders($userid, $perm = PERMS_SHOW, $parent = null, $allLevels = true)
    {
    ...
    function addFolder($folder)
    {
        ...
        $perm->addUserPermission($folder->get('owner'), PERMS_SHOW, false);
        $perm->addUserPermission($folder->get('owner'), PERMS_READ, false);
        $perm->addUserPermission($folder->get('owner'), PERMS_EDIT, false);
        $perm->addUserPermission($folder->get('owner'), PERMS_DELETE, false);

        $folder->setPermission($perm, false);

        $result = Horde::callHook('_horde_hook_share_add', array($folder),
                                  'horde', false);
        if (is_a($result, 'PEAR_Error')) {
            return $result;
        }
    ...
    function removeFolder($folder, $force = false)
    {
        ...
        $result = Horde::callHook('_horde_hook_share_remove', array($folder),
                                  'horde', false);
        if (is_a($result, 'PEAR_Error')) {
            return $result;
        }
    ...
    function removeBookmark($bookmark)
    {
        ...
        if (!$folder->hasPermission(Auth::getAuth(), PERMS_DELETE)) {
    ...
    function resultSet($bookmarks)
    {
        ...
        foreach ($bookmarks as $bookmark) {
            ...
            if (!empty($value) && !is_numeric($value)) {
                $cvBookmarks[$key] = String::convertCharset(
     ...
     */
    function _getShareCriteria($userid, $perm = PERMS_SHOW)
    {
        if (!empty($userid)) {
            $criteria = array(
                'OR' => array(
...
class Trean_Bookmark {
    ...
    function Trean_Bookmark($bookmark = array())
    {
        if ($bookmark) {
            ...
            if (!empty($bookmark['favicon_id'])) {
                $this->favicon = (int)$bookmark['favicon_id'];
            }
        }
    }
    ...
    function save()
    {
        if ($this->id) {
            ...
            $update = $GLOBALS['trean_db']->prepare('
                ...
                WHERE bookmark_id = ?',
                array('integer', 'text', 'text', 'text', 'integer', 'integer', 'integer', 'text', 'integer')
            );
            if (is_a($update, 'PEAR_Error')) {
                return $update;
            }
            $result = $update->execute(array($this->folder,
                                             String::convertCharset($this->url, NLS::getCharset(), $GLOBALS['conf']['sql']['charset']),
                                             String::convertCharset($this->title, NLS::getCharset(), $GLOBALS['conf']['sql']['charset']),
                                             String::convertCharset($this->description, NLS::getCharset(), $GLOBALS['conf']['sql']['charset']),
                                             $this->clicks,
                                             $this->rating,
                                             $this->favicon,
                                             String::convertCharset($this->http_status, NLS::getCharset(), $GLOBALS['conf']['sql']['charset']),
                                             $this->id));
            if (is_a($result, 'PEAR_Error')) {
                Horde::logMessage($result, __FILE__, __LINE__, PEAR_LOG_ERR);
            }
        ...
        $insert = $GLOBALS['trean_db']->prepare('
            INSERT INTO trean_bookmarks
                (bookmark_id, folder_id, bookmark_url, bookmark_title, bookmark_description,
                 bookmark_clicks, bookmark_rating, favicon_id, bookmark_http_status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            array('integer', 'integer', 'text', 'text', 'text', 'integer', 'integer', 'integer', 'text')
        );
