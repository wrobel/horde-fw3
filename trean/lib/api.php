 ...
 *
 * $Horde: trean/lib/api.php,v 1.23.2.1 2008/11/22 19:33:53 chuck Exp $
 *
...
$_services['getFolders'] = array(
    'args' => array('folderId' => 'integer'),
    'type' => 'array',
);

$_services['updateObjects'] = array(
    'args' => array('data' => 'array'),
    'type' => 'boolean',
);

$_services['addObjects'] = array(
    'args' => array('objects' => 'array'),
    'type' => 'array',
);

$_services['listBookmarks'] = array(
    'args' => array('folderId' => 'int',
                    'sortby' => 'string',
                    'sortdir' => 'int',
                    'from' => 'int',
                    'count' => 'int'),
...
$_services['deleteFolder'] = array(
    'args' => array('folderId' => 'int',
                    'force' => 'boolean'),
    'type' => 'boolean',
);

$_services['deleteFolders'] = array(
    'args' => array('Ids' => 'array'),
    'type' => 'boolean',
);

$_services['deleteBookmark'] = array(
    'args' => array('bookmarkId' => 'int'),
    'type' => 'boolean',
);

$_services['deleteBookmarks'] = array(
    'args' => array('Ids' => 'array'),
    'type' => 'boolean',
);

$_services['syncFolders'] = array(
    'args' => array('folderId' => 'int',
                    'folderIds' => 'array'),
    'type' => 'array',
);

$_services['syncBookmarks'] = array(
    'args' => array('folderId' => 'int',
                    'bookmarkIds' => 'array'),
    'type' => 'array',
);

$_services['getAddUrl'] = array(
 ...
 */
function _trean_getFolders($folderId)
{
    ...
    $folderObs = Trean::listFolders(PERMS_SHOW,
 ...
 */
function _trean_addObjects($data)
{
    require_once dirname(__FILE__) . '/base.php';
    $return_map = array();
    ...
    foreach ($data as $props) {
        ...
        if ($children) {
            ...
            $ret = _trean_addObjects($children);
            if (is_a($ret, 'PEAR_Error')) {
                return $ret;
            }
 ...
 * @param array $data   Object data
 */
function _trean_updateObjects($data)
{
    require_once dirname(__FILE__) . '/base.php';
 ...
 */
function _trean_listBookmarks($folderId, $sortby = 'title', $sortdir = 0, $from = 0, $count = 0)
{
    require_once dirname(__FILE__) . '/base.php';
 ...
 */
function _trean_deleteFolder($folderId, $force)
{
    require_once dirname(__FILE__) . '/base.php';
 ...
 * @return boolean  True for success.
 */
function _trean_deleteFolders($Ids)
{
    $Ids = array_reverse($Ids);
    foreach ($Ids as $Id) {
        $ret = _trean_deleteFolder($Id, true);
        if (is_a($ret, 'PEAR_Error')) {
            return $ret;
        }
    }
 ...
 * @return boolean  True for success.
 */
function _trean_deleteBookmark($bookmarkId)
{
    require_once dirname(__FILE__) . '/base.php';
 ...
 * @return boolean  True for success.
 */
function _trean_deleteBookmarks($Ids)
{
    foreach ($Ids as $Id) {
        $ret = _trean_deleteBookmark($Id);
        if (is_a($ret, 'PEAR_Error')) {
            return $ret;
       }
    }
 ...
 * @see _trean_listBookmarks()
 */
function _trean_syncBookmarks($folderId, $bookmarkIds)
{
    require_once dirname(__FILE__) . '/base.php';

    $rawbookmarks = _trean_listBookmarks($folderId);
 ...
 * @see _trean_getFolders()
 * @see _trean_syncBookmarks()
 */
function _trean_syncFolders($folderId, $folderIds)
{
    require_once dirname(__FILE__) . '/base.php';

    $rawfolders = _trean_getFolders($folderId);
 ...
 */
function _trean_getAddUrl($params = array())
{
    require_once dirname(__FILE__) . '/base.php';
    ...
    if ($GLOBALS['browser']->hasFeature('javascript')) {
        if ($browser->hasFeature('dom')) {
            $addurl = Util::addParameter(Horde::applicationUrl('add.php', true, -1), 'iframe', 1);
            $url = "javascript:(function(){o=document.createElement('div');o.id='_trean_overlay';o.style.background='#000';o.style.position='absolute';o.style.top=0;o.style.left=0;o.style.width='100%';o.style.height='100%';o.style.zIndex=5000;o.style.opacity=.8;document.body.appendChild(o);i=document.createElement('iframe');i.id='_trean_frame';i.style.zIndex=5001;i.style.border='thin solid #000';i.src='$addurl'+'&title=' + encodeURIComponent(document.title) + '&url=' + encodeURIComponent(location.href);i.style.position='absolute';i.style.width='350px';i.style.height='150px';i.style.left='100px';i.style.top='100px';document.body.appendChild(i);l=document.createElement('a');l.style.position='absolute';l.style.background='#ccc';l.style.color='#000';l.style.border='thin solid #000';l.style.display='block';l.style.top='250px';l.style.left='100px';l.style.zIndex=5001;l.style.padding='5px';l.appendChild(document.createTextNode('" . _("Close") . "'));l.onclick=function(){var o=document.getElementById('_trean_overlay');o.parentNode.removeChild(o);var i=document.getElementById('_trean_frame');i.parentNode.removeChild(i);this.parentNode.removeChild(this);};document.body.appendChild(l);})()";
        } else {
            $addurl = Horde::applicationUrl(Util::addParameter('add.php', 'popup', 1), true, -1);
        ...
        $url = Horde::applicationUrl(Util::addParameter('add.php', $params), true);
    }
