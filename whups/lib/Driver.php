...
class Whups_Driver {
    ...
    function setAttributes($info, &$ticket)
    {
        ...
        foreach ($info as $name => $value) {
            if (substr($name, 0, 10) == 'attribute_' &&
                ...
                $this->updateLog($ticket_id, Auth::getAuth(), array('attribute' => $attribute_id . ':' . $value));
            }
        }
    }
                  ...
                  $reminder, $queue_id = null, $is_new = false)
    {
        ...
        require_once WHUPS_BASE . '/lib/version.php';
        ...
        $mail->addHeader('User-Agent', 'Whups ' . WHUPS_VERSION);
        ...
        foreach ($recipients as $user) {
            if ($user == $from && $user == Auth::getAuth() &&
            ...
            if (!empty($mail_always) && $user == $mail_always) {
                ...
                if (!empty($details['email'])) {
                    ...
                    $mycomments = Whups::permissionsFilter(
                        $comments, 'comment', PERMS_READ, $details['user']);
                }
            ...
            $addr_arr = MIME::parseAddressList($to);
            ...
            $body = str_replace(
                ...
                $message);
            $mail->setBody($body, NLS::getCharset());

            $mail->addHeader('Message-ID', MIME::generateMessageID());
            if ($ticket_id) {
            ...
            $result = $mail->send($mail_driver, $mail_params, true);
