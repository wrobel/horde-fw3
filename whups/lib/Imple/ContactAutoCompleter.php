<?php
/**
 * $Horde: whups/lib/Imple/ContactAutoCompleter.php,v 1.1.2.1 2009/01/06 15:28:23 jan Exp $
 *
...
class Imple_ContactAutoCompleter extends Imple {
    ...
    {
        parent::attach();
        $url = Horde::url($GLOBALS['registry']->get('webroot', 'whups') . '/imple.php?imple=ContactAutoCompleter/input=' . rawurlencode($this->_params['triggerId']), true);
        Whups::addInlineScript('Event.observe(window, "load", function() { new Ajax.Autocompleter("' . $this->_params['triggerId'] . '", "' . $this->_params['resultsId'] . '", "' . $url . '", { tokens: ",", indicator: "' . $this->_params['triggerId'] . '_loading_img"' . $this->_onshow . ' }); });');
    }
    ...
    function handle($args)
    {
        ...
        $results = $this->expandAddresses($input, false);
