 ...
 *
 * $Horde: whups/lib/Forms/Admin/Queue.php,v 1.2.2.1 2009/01/06 15:28:21 jan Exp $
 *
    ...
    function EditQueueStep2Form(&$vars)
    {
        ...
        if ($registry->hasMethod('tickets/listVersions') == $registry->getApp()) {
            $versionlink = array(
                ...
                'url' => Util::addParameter(Horde::applicationUrl('admin/?formname=editversionstep1form'), 'queue', $queue));
        ...
        $userlink = array(
            ...
            'url' => Util::addParameter(Horde::applicationUrl('admin/?formname=edituserform'), 'queue', $queue));
        $this->addVariable('', 'link', 'link', false, true, null,
                           array($userlink));

        /* Perms link. */
        if (Auth::isAdmin('whups:admin', PERMS_EDIT)) {
            $permslink = array(
                'text' => _("Edit the permissions on this queue"),
                'url' => Util::addParameter(Util::addParameter(Horde::url($GLOBALS['registry']->get('webroot', 'horde') . '/admin/perms/edit.php'), 'category', "whups:queues:$queue"), 'autocreate', '1'));
            $this->addVariable('', 'link', 'link', false, true, null,
                               array($permslink));
        }
    }
