 ...
 *
 * $Horde: whups/lib/Forms/EditTicket.php,v 1.22.2.1 2009/01/23 16:59:53 chuck Exp $
 *
...
class EditTicketForm extends Horde_Form {
    ...
    function EditTicketForm(&$vars, &$ticket, $title = '')
    {
        ...
        if (empty($GLOBALS['conf']['hooks']['group_fields'])) {
            ...
            $grouped_fields = Horde::callHook('_whups_hook_group_fields',
                                              array($ticket->get('type'),
                                                    $fields),
                                              'whups');
            if (is_a($grouped_fields, 'PEAR_Error')) {
                Horde::logMessage($grouped_fields, __FILE__, __LINE__, PEAR_LOG_ERR);
                $grouped_fields = array($fields);
            }
        }
