 ...
 * SearchForm Class.
 *
 * $Horde: whups/lib/Forms/Search.php,v 1.10 2008/09/15 12:21:21 jan Exp $
 *
...
class SearchForm extends Horde_Form {
    ...
    function SearchForm(&$vars)
    {
        ...
        $queues = Whups::permissionsFilter($GLOBALS['whups_driver']->getQueues(), 'queue', PERMS_READ);
    ...
    function getInfo($vars, &$info)
    {
        ...
        if (empty($info['queue'])) {
            $info['queue'] = array_keys(Whups::permissionsFilter($GLOBALS['whups_driver']->getQueues(), 'queue', PERMS_READ, Auth::getAuth(), Auth::getAuth()));
        }
