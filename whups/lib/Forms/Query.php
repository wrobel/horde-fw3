 ...
 *
 * $Horde: whups/lib/Forms/Query.php,v 1.12.2.1 2009/01/06 15:28:20 jan Exp $
 *
...
class ChooseQueryNameForLoadForm extends Horde_Form {
    ...
    function ChooseQueryNameForLoadForm(&$vars)
    {
        ...
        $qParams = $qManager->listQueries(Auth::getAuth());
        if (count($qParams)) {
            $qType = 'enum';
        } else {
            $qType = 'invalid';
...
class DeleteQueryForm extends Horde_Form {
    ...
    function execute(&$vars)
    {
        ...
        if ($vars->get('yesno')) {
            if (!$GLOBALS['whups_query']->hasPermission(Auth::getAuth(), PERMS_DELETE)) {
