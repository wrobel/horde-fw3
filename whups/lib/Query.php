<?php
/**
 * $Horde: whups/lib/Query.php,v 1.73.2.2 2009/01/06 15:28:17 jan Exp $
 *
...
class Whups_Query {
    ...
    function getTabs($vars)
    {
        ...
        $edit = $this->hasPermission(Auth::getAuth(), PERMS_EDIT);
        $delete = $this->hasPermission(Auth::getAuth(), PERMS_DELETE);
        ...
        if ($this->id && $edit && empty($GLOBALS['conf']['share']['no_sharing'])) {
            ...
            $permsurl = Util::addParameter($permsurl, array('app' => 'whups',
                                                            'cid' => $this->id));
...
class Whups_QueryManager {
    ...
    {
        $shares = $this->_shareManager->listShares(Auth::getAuth(), PERMS_READ,
                                                   array('slug' => $slug));
