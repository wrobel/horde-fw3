<?php
/**
 * $Horde: whups/lib/Whups.php,v 1.185.2.2 2009/01/06 15:28:17 jan Exp $
 *
...
class Whups {
    ...
    function urlFor($controller, $data, $full = false, $append_session = 0)
    {
        ...
        case 'query_rss':
            if ($rewrite) {
                ...
                $url = Util::addParameter($url, $param);
                return Horde::applicationUrl($url, $full, $append_session);
            }
            break;
        }
    ...
    {
        $id = preg_replace('|\D|', '', Util::getFormData('id'));
        if (!$id) {
    ...
    function getTicketTabs(&$vars, $id)
    {
        ...
        $tabs->addTab(_("_Watch"),
                      ...
                      'watch');
        if (Whups::hasPermission($queue, 'queue', PERMS_DELETE)) {
    ...
    function hasPermission($in, $filter, $permission, $user = null)
    {
        ...
        if ($permission == 'update' ||
            ...
            $admin = Auth::isAdmin('whups:admin', PERMS_EDIT, $user);
        } else {
            $admin = Auth::isAdmin('whups:admin', $permission, $user);
        }
                        ...
                        if ($change['type'] != 'comment' ||
                            ...
                            if (isset($change['comment'])) {
                                $out[$key]['comment_text'] = $change['comment'];
                                  ...
                                  $perms->hasPermission('whups:comments:'
                                                        . $change['value'],
                                                        $user, PERMS_READ,
                                                        $creator)) {
                            $change['private'] = true;
        ...
        if (!isset($results[$user])) {
            ...
            if ($type == 'user') {
                if (substr($user, 0, 2) == '**') {
                    ...
                    $addr_arr = MIME::parseAddressList($user);
                    ...
                    require_once 'Horde/MIME.php';
                    $addr_arr = MIME::parseAddressList($results[$user]['email']);
                    if (!is_a($addr_arr, 'PEAR_Error') &&
                        isset($addr_arr[0])) {
                        $results[$user]['name'] = isset($addr_arr[0]->personal)
                            ? $addr_arr[0]->personal : '';
     ...
     *
     * @param Variables &$vars  The selection criteria.
     */
    ...
    function attachmentUrl($ticket, $file, $queue)
    {
        ...
        $url_params = array('actionID' => 'download_file',
                            'file' => $file['name'],
                            'ticket' => $ticket);
        $link .= ' ' . Horde::link(Horde::downloadUrl($file['name'], $url_params), $file['name']) . Horde::img('download.png', _("Download"), null, $registry->getImageDir('horde')) . '</a>';
        ...
        if (Whups::hasPermission($queue, 'queue', PERMS_DELETE)) {
            ...
            $link .= ' ' . Horde::link($url, sprintf(_("Delete %s"), $file['name']), '', '', 'return window.confirm(\'' . addslashes(sprintf(_("Permanently delete %s?"), $file['name'])) . '\');') .
                Horde::img('delete.png', sprintf(_("Delete %s"), $file['name']), null, $registry->getImageDir('horde')) . '</a>';
        }
