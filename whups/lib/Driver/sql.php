...
class Whups_Driver_sql extends Whups_Driver {
    ...
    function addState($typeId, $name, $description, $category)
    {
        ...
        $values = array($new_id,
                        ...
                        String::convertCharset($description, NLS::getCharset(),
                                               $this->_params['charset']),
                        String::convertCharset($category, NLS::getCharset(),
                                               $this->_params['charset']));
        Horde::logMessage(
    ...
    function addReply($type, $name, $text)
    {
        ...
        $values = array($type,
                        ...
                        String::convertCharset($name, NLS::getCharset(),
                                               $this->_params['charset']),
                        String::convertCharset($text, NLS::getCharset(),
                                               $this->_params['charset']));
        Horde::logMessage(
    ...
    function addTicket(&$info, $requester)
    {
        ...
        $values = array($ticketId,
                        String::convertCharset($summary, NLS::getCharset(),
                                               $this->_params['charset']),
                        $requester,
                        $type,
                        $state,
    ...
    function addComment($ticket_id, $comment, $creator, $creator_email = null)
    {
        ...
        $result = $this->_write_db->query('INSERT INTO whups_comments (comment_id, ticket_id, user_id_creator, comment_text, comment_timestamp)' .
                                    ...
                                    array((int)$id,
                                          (int)$ticket_id,
                                          $creator,
                                          String::convertCharset($comment, NLS::getCharset(), $this->_params['charset']),
                                          time()));
        if (is_a($result, 'PEAR_Error')) {
            Horde::logMessage($result, __FILE__, __LINE__, PEAR_LOG_ERR);
            return $result;
    ...
    function updateTicket($ticketId, $attributes)
    {
        ...
        foreach ($attributes as $field => $value) {
            ...
            $values[] = String::convertCharset($value, NLS::getCharset(), $this->_params['charset']);
        }
    ...
    function getTicketsByProperties($info, $munge = true, $perowner = false)
    {
        ...
        if (!empty($info['summary'])) {
            $where = $this->_addWhere(
                ...
                . $this->_write_db->quote('%' . String::lower($info['summary']) . '%'));
        }
        ...
        $info = String::convertCharset($info, $this->_params['charset']);
    ...
    function getTicketDetails($ticket, $checkPerms = true)
    {
        ...
        if (is_a($result, 'PEAR_Error')) {
            ...
            $queues = Whups::permissionsFilter(
                $this->getQueues(), 'queue', PERMS_READ, Auth::getAuth(),
    ...
    function getGuestEmail($guest_id)
    {
        ...
        if (!isset($guestCache[$guest_id])) {
            ...
            $guestCache[$guest_id] = String::convertCharset(
    ...
    function _getHistory($ticket_id)
    {
        ...
        $history = String::convertCharset($history, $this->_params['charset']);
    ...
    function getQueueSummary($queue_ids)
    {
        ...
        return String::convertCharset($queues, $this->_params['charset']);
    }
    ...
    function getQueueInternal($queueId)
    {
        ...
        $queue = String::convertCharset($queue, $this->_params['charset']);
    ...
    function getQueuesInternal()
    {
        ...
        $internals = String::convertCharset($queues, $this->_params['charset']);
        return $internals;
    }
                         ...
                         $slug = '', $email = '', $default = null)
    {
        ...
        if ($registry->hasMethod('tickets/listQueues') == $registry->getApp()) {
            ...
            $values = array(String::convertCharset($name,
                                                   NLS::getCharset(),
                                                   $this->_params['charset']),
                            String::convertCharset($description,
                                                   NLS::getCharset(),
                                                   $this->_params['charset']),
                            (empty($versioned) ? 0 : 1),
                            $slug,
    ...
    function updateQueueUsers($queueId, $users)
    {
        // clear all previous type-queue associations
        $query = 'DELETE FROM whups_modules_users WHERE queue_id = ?';
        $values = array($queueId);
        Horde::logMessage(
            sprintf('Whups_Driver_sql::updateQueueUser(): query="%s"; values="%s"',
                    $query, implode(',', $values)),
            __FILE__, __LINE__, PEAR_LOG_DEBUG);
        $result = $this->_write_db->query($query, $values);
        if (is_a($result, 'PEAR_Error')) {
        ...
        // add the new associations
        foreach ($types as $typeId) {
            $query = 'INSERT INTO whups_queues_users (queue_id, type_id) '
                . 'VALUES (?, ?)';
            $values = array($queueId, $typeId);
            Horde::logMessage(
                sprintf('Whups_Driver_sql::updateQueueUser(): query="%s"; values="%s"',
                        $query, implode(',', $values)),
                __FILE__, __LINE__, PEAR_LOG_DEBUG);
    ...
    function getType($typeId)
    {
        ...
        $type = String::convertCharset($type, $this->_params['charset']);
    ...
    function getTypes($queueId)
    {
        ...
        return String::convertCharset($types, $this->_params['charset']);
    }
    ...
    function getAllTypeInfo()
    {
        ...
        return String::convertCharset($info, $this->_params['charset']);
    }
    ...
    function getTypeName($type)
    {
        ...
        return String::convertCharset($name, $this->_params['charset']);
    }
    ...
    function getStates($type = null, $category = '', $notcategory = '')
    {
        ...
        return String::convertCharset($states, $this->_params['charset']);
    }
    ...
    function getState($stateId)
    {
        ...
        $state = String::convertCharset($state, $this->_params['charset']);
        return array(
            'id' => $stateId,
    ...
    function getQuery($queryId)
    {
        ...
        return String::convertCharset($query, $this->_params['charset']);
    }
    ...
    function saveQuery($query)
    {
        ...
        $values = String::convertCharset($values, NLS::getCharset(),
                                         $this->_params['charset']);
        Horde::logMessage(
    ...
    function getPriorities($type = null)
    {
        ...
        return String::convertCharset($priorities, $this->_params['charset']);
    }
    ...
    function getPriority($priorityId)
    {
        ...
        $priority = String::convertCharset($priority, $this->_params['charset']);
    ...
    function getVersionInternal($versionId)
    {
        ...
        $version = String::convertCharset($version, $this->_params['charset']);
    ...
    function getReply($reply_id)
    {
        ...
        return String::convertCharset($reply, $this->_params['charset']);
    }
    ...
    function addAttributeDesc($type_id, $name, $desc, $type, $params, $required)
    {
        ...
        $values = array($new_id,
                        ...
                        String::convertCharset($name, NLS::getCharset(),
                                               $this->_params['charset']),
                        String::convertCharset($desc, NLS::getCharset(),
                                               $this->_params['charset']),
                        $type,
                        serialize(
                            String::convertCharset($params, NLS::getCharset(),
                                                   $this->_params['charset'])),
                        (int)($required == 'on'));
                                 ...
                                 $newparams, $newrequired)
    {
        ...
        $values = array(String::convertCharset($newname, NLS::getCharset(),
                                               $this->_params['charset']),
                        String::convertCharset($newdesc, NLS::getCharset(),
                                               $this->_params['charset']),
                        $newtype,
                        serialize(
                            String::convertCharset($newparams,
                                                   NLS::getCharset(),
                                                   $this->_params['charset'])),
                        (int)($newrequired == 'on'),
                        $attribute_id);
    ...
    function getAllAttributes()
    {
        ...
        return String::convertCharset($attributes, $this->_params['charset']);
    }
    ...
    function getAttributeDesc($attribute_id)
    {
        ...
        return array(
            'id' => $attribute_id,
            'attribute_name' => String::convertCharset(
                $attribute['attribute_name'], $this->_params['charset']),
            'attribute_description' => String::convertCharset(
            ...
            'attribute_params' => String::convertCharset(
    ...
    function _getAttributesForType($type = null, $raw = false)
    {
        ...
        foreach ($attributes as $id => $attribute) {
            ...
            $attributes[$id]['attribute_name'] = String::convertCharset(
                $attribute['attribute_name'], $this->_params['charset']);
            $attributes[$id]['attribute_description'] = String::convertCharset(
                $attribute['attribute_description'], $this->_params['charset']);
            $attributes[$id]['attribute_type'] =
                empty($attribute['attribute_type'])
                ? 'text' : $attribute['attribute_type'];
            $attributes[$id]['attribute_params'] = String::convertCharset(
                @unserialize($attribute['attribute_params']),
                $this->_params['charset']);
    ...
    function getAttributeNamesForType($type_id)
    {
        ...
        return String::convertCharset($names, $this->_params['charset']);
    }
    ...
    {
        $db_attribute_value = String::convertCharset((string)$attribute_value,
                                                     NLS::getCharset(),
                                                     $this->_params['charset']);
                       ...
                       $transactionId = null)
    {
        ...
        foreach ($changes as $type => $value) {
            ...
            $values = array(
                ...
                (string)$user,
                $type,
                String::convertCharset((string)$value, NLS::getCharset(),
                                       $this->_params['charset']),
                (int)$value);
            Horde::logMessage(
