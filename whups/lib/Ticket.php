...
class Whups_Ticket {
    ...
    function commit($user = null, $transaction = null, $notify = true)
    {
        ...
        if (!empty($GLOBALS['conf']['hooks']['update'])) {
            $changes = Horde::callHook('_whups_hook_ticket_update',
                                       array($this, $this->_changes), 'whups');
    ...
    function notify($author, $isNew, $listeners = array())
    {
        ...
        foreach ($field_names as $field_name) {
            $length = max($length, String::length($field_name));
        }
        $wrap_break = "\n" . str_repeat(' ', $length + 2) . '| ';
        ...
        /* Ticket properties. */
        $table = "------------------------------------------------------------------------------\n"
            . ' ' . String::pad(_("Ticket"), $length) . ' | '
            . $this->_id . "\n" . ' '
            . String::pad($isNew ? _("Created By") : _("Updated By"), $length)
            . ' | ' . Whups::formatUser($author) . "\n";
        if (isset($this->_changes['summary'])) {
            $table .= '-' . String::pad(_("Summary"), $length) . ' | '
                . String::wrap($this->_changes['summary']['from'],
                               $wrap_width, $wrap_break)
                . "\n" . '+' . String::pad(_("Summary"), $length) . ' | '
                . String::wrap($this->get('summary'), $wrap_width, $wrap_break)
                . "\n";
        } else {
            $table .= ' ' . String::pad(_("Summary"), $length) . ' | '
                . String::wrap($this->get('summary'), $wrap_width, $wrap_break)
        ...
        foreach ($fields as $field => $label) {
            if ($name = $this->get($field . '_name')) {
                if (isset($this->_changes[$field])) {
                    $table .= '-' . String::pad($label, $length) . ' | '
                        . String::wrap($this->_changes[$field]['from_name'],
                                       $wrap_width, $wrap_break)
                        . "\n" . '+' . String::pad($label, $length) . ' | '
                        . String::wrap($name, $wrap_width, $wrap_break) . "\n";
                } else {
                    $table .= ' ' . String::pad($label, $length) . ' | '
                        . String::wrap($name, $wrap_width, $wrap_break) . "\n";
                }
        ...
        foreach ($attributes as $id => $attribute) {
            ...
            if (isset($this->_changes[$attribute_id])) {
                $table .= '-' . String::pad($label, $length) . ' | '
                    . String::wrap($this->_changes[$attribute_id]['from'],
                                   $wrap_width, $wrap_break)
                    . "\n" . '+' . String::pad($label, $length) . ' | '
                    . String::wrap($this->_changes[$attribute_id]['to'],
                                   $wrap_width, $wrap_break)
                    . "\n";
            } else {
                $table .= ' ' . String::pad($label, $length) . ' | '
                    . String::wrap($this->get($attribute_id),
                                   $wrap_width, $wrap_break)
                    . "\n";
            }
        }
        ...
        $owners = $oldOwners = String::wrap(
            Whups::getOwners($this->_id, false, true),
            $wrap_width, $wrap_break);
        if (isset($this->_changes['oldowners'])) {
            $oldOwners = String::wrap(
                Whups::getOwners($this->_id, false, true,
                                 $this->_changes['oldowners']),
                $wrap_width, $wrap_break);
        }
        if ($owners != $oldOwners) {
            $table .= '-' . String::pad(_("Owners"), $length) . ' | '
                . $oldOwners . "\n" . '+' . String::pad(_("Owners"), $length)
                . ' | ' . $owners . "\n";
        } else {
            $table .= ' ' . String::pad(_("Owners"), $length) . ' | '
                . $owners . "\n";
        ...
        if (isset($this->_changes['attachment'])) {
            $table .= '+' . String::pad(_("New Attachment"), $length) . ' | '
                . $this->_changes['attachment']['to']['name'] . "\n";
        }

        /* Deleted Attachments. */
        if (isset($this->_changes['delete-attachment'])) {
            $table .= '+' . String::pad(_("Deleted Attachment"), $length)
        ...
        $message = String::convertCharset(file_get_contents($message_file),
                                          'UTF-8');
        $message = str_replace(
            array('@@ticket_url@@',
                  '@@table@@',
    ...
    function toString()
    {
        ...
        foreach (array_merge($fields, array(_("Summary"), _("Owners")))
                 as $field) {
            $length = max($length, String::length($field));
        }
        $wrap_break = "\n" . str_repeat(' ', $length + 2) . '| ';
        $wrap_width = 73 - $length;

        /* Ticket properties. */
        $message = ' ' . String::pad(_("Ticket"), $length) . ' | '
            . $this->_id . "\n" . ' ' . String::pad(_("Summary"), $length)
            . ' | ' . String::wrap($this->get('summary'),
                                   $wrap_width, $wrap_break)
            . "\n";

        foreach ($fields as $field => $label) {
            if ($name = $this->get($field . '_name')) {
                $message .= ' ' . String::pad($label, $length) . ' | '
                    . String::wrap($name, $wrap_width, $wrap_break) . "\n";
            }
        }

        $message .= ' ' . String::pad(_("Owners"), $length) . ' | '
            . String::wrap(Whups::getOwners($this->_id, false, true),
                           $wrap_width, $wrap_break)
            . "\n";
...
class TicketDetailsForm extends Horde_Form {
    ...
    function TicketDetailsForm(&$vars, &$ticket, $title = '')
    {
        ...
        if (empty($GLOBALS['conf']['hooks']['group_fields'])) {
            $grouped_fields = array($fields);
        } else {
            $grouped_fields = Horde::callHook(
                '_whups_hook_group_fields',
                array($ticket->get('type'), $fields),
                'whups');
            if (is_a($grouped_fields, 'PEAR_Error')) {
                Horde::logMessage($grouped_fields, __FILE__, __LINE__,
                                  PEAR_LOG_ERR);
                $grouped_fields = array($fields);
            }
        }
