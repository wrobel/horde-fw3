<?php
/**
 * $Horde: whups/search.php,v 1.124.2.1 2009/01/06 15:28:14 jan Exp $
 *
...
function _getSearchUrl($vars)
{
    ...
    $qUrl = Util::addParameter($qUrl, array('queue' => $queue));

    $summary = $vars->get('summary');
    if ($summary) {
        $qUrl = Util::addParameter($qUrl, 'summary', $summary);
    }
    ...
    if (is_array($states)) {
        foreach ($states as $type => $state) {
            if (is_array($state)) {
                foreach ($state as $s) {
                    $qUrl = Util::addParameter($qUrl, "states[$type][]", $s);
                }
            } else {
                $qUrl = Util::addParameter($qUrl, "states[$type]", $state);
            }
        }
    }
...
if (($vars->get('formname') || $vars->get('summary') || $vars->get('states') ||
    ...
    if ($vars->get('submitbutton') == _("Save as Query")) {
        ...
        header('Location: ' . Horde::applicationUrl(Util::addParameter('query/index.php', 'action', 'save'), true));
        exit;
    }
...
$myqueries = Whups_View::factory(
    'SavedQueries',
    array('title' => Auth::getAuth() ? _("My Queries") : _("Public Queries"),
          'results' => $qManager->listQueries(Auth::getAuth(), true)));
$myqueries->html();
