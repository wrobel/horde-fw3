 ...
 *
 * $Horde: jonah/channels/index.php,v 1.43 2008/05/31 21:14:59 chuck Exp $
 *
...
if (is_a($channels, 'PEAR_Error')) {
    ...
    $channels = Jonah::checkPermissions('channels', PERMS_SHOW, $channels);
    ...
    foreach ($channels as $key => $channel) {
        ...
        /* List recipients for delivery of this channel. */
        $url = Horde::applicationUrl('lists/index.php');
        $url = Util::addParameter($url, 'channel_id', $channel['channel_id']);
        $channels[$key]['lists_url'] = $url;
        $channels[$key]['lists_link'] = Horde::link($url, _("Email subscribers"), '', '', '', _("Email subscribers")) . Horde::img('lists.png', _("Email subscribers")) . '</a>';
        ...
        case JONAH_AGGREGATED_CHANNEL:
            /* Refresh cache link. */
            $url = Horde::applicationUrl('stories/index.php');
            $url = Util::addParameter($url, array('channel_id' => $channel['channel_id'], 'refresh' => '1', 'url' => Horde::selfUrl()));
