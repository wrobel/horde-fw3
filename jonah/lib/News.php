 ...
 *
 * $Horde: jonah/lib/News.php,v 1.154 2008/05/29 20:40:49 chuck Exp $
 *
...
class Jonah_News {
        ...
        if (!isset($channel[$channel_id])) {
            ...
            if (!is_a($channel[$channel_id], 'PEAR_Error')) {
                if (empty($channel[$channel_id]['channel_link'])) {
                    $channel[$channel_id]['channel_official'] = Util::addParameter(Horde::applicationUrl('delivery/html.php', true, -1), 'channel_id', $channel_id, false);
                } else {
                                 ...
                                 $order = JONAH_ORDER_PUBLISHED)
    {
        ...
        $channel['channel_link'] = Util::addParameter(Horde::applicationUrl('delivery/html.php', true, -1), 'channel_id', $channel['channel_id']);
        $stories = $this->_getStories($channel['channel_id'], $max, $from, $date, $unreleased, $order);
    ...
    function saveStory(&$info)
    {
        ...
        /* Check if there are distribution lists for this channel and send out
         * any distributions required. */
        // TODO: don't deliver if there are no subscribers.
        require_once JONAH_BASE . '/lib/Delivery.php';
        ...
        foreach ($delivery_drivers as $driver => $desc) {
            ...
            $driver_info = $delivery->getInfo();
            $result = $delivery->singleDeliver($info);
            if (is_a($result, 'PEAR_Error')) {
                if (isset($GLOBALS['notification'])) {
                    $GLOBALS['notification']->push(sprintf(_("Could not deliver to the %s distribution list. %s"), $driver_info['name'], $result->getMessage()), 'horde.error');
                } else {
                    Horde::logMessage($result, __FILE__, __LINE__, PEAR_LOG_ERR);
                }
            } elseif ($result && isset($GLOBALS['notification'])) {
                $GLOBALS['notification']->push(sprintf(_("Successfully delivered to the %s distribution list."), $driver_info['name']), 'horde.success');
            }
        }
    }
    ...
    function getStoryLink($channel, $story)
    {
        if ((empty($story['story_url']) || !empty($story['story_body'])) &&
            ...
            $url = Util::addParameter(Horde::applicationUrl('stories/view.php', true, -1), array('channel_id' => '%c', 'story_id' => '%s'), null, false);
        }
    ...
    function fetchExternalStories($channel_id, $url, $interval)
    {
        ...
        if (is_a($cache, 'Horde_Cache') && ($stories = $cache->get($url, $interval))) {
            $stories = Horde_Serialize::unserialize($stories, SERIALIZE_UTF7_BASIC, NLS::getCharset());
        } else {
            $stories = Jonah_News::_fetchExternalStories($url, $timestamp);
            $cache->set($url, Horde_Serialize::serialize($stories, SERIALIZE_UTF7_BASIC, NLS::getCharset()));
        }
    ...
    function &getStoryAsMessage(&$story)
    {
        ...
        case 'richtext':
            ...
            $body_text = Text_Filter::filter($body_html, 'html2text');

            /* Add description. */
            $body_html = '<p>' . Text_Filter::filter($story['story_desc'], 'text2html', array('parselevel' => TEXT_HTML_MICRO, 'charset' => NLS::getCharset(), 'class' => null, 'callback' => null)) . "</p>\n" . $body_html;
            $body_text = String::wrap('  ' . $story['story_desc'], 70) . "\n\n" . $body_text;

            /* Add the text version of the story to the base message. */
            $message_text = new MIME_Part('text/plain');
            $message_text->setCharset(NLS::getCharset());
            ...
            $message_html = new MIME_Part('text/html', String::wrap($body_html),
                                          NLS::getCharset(), 'inline');
    ...
    function _fetchExternalStories($url, $timestamp)
    {
        ...
        if (!empty($stories['items'])) {
            foreach ($stories['items'] as $key => $story) {
                ...
                $items[$key]['story_title'] = isset($story['title']) ? String::convertCharset($story['title'], 'utf-8') : _("[No title]");
                $items[$key]['story_desc'] = isset($story['description']) ? String::convertCharset($story['description'], 'utf-8') : '';
                $items[$key]['story_url'] = isset($story['link']) ? $story['link'] : '';

                /* Set the body, and filter it if it's HTML */
                $items[$key]['story_body'] = isset($story['body']) ? String::convertCharset($story['body'], 'utf-8') : null;
                $items[$key]['story_body_type'] = isset($story['body_type']) ? String::convertCharset($story['body_type'], 'utf-8') : 'text';
                if ($items[$key]['story_body_type'] == 'html') {
                    require_once 'Horde/Text/Filter.php';
                    $items[$key]['story_body'] = Text_Filter::filter($items[$key]['story_body'], 'xss');
                }
                ...
                if (isset($story['media:content']) && is_array($story['media:content'])) {
                    ...
                    $items[$key]['story_media_content_type'] =$story['media:content']['type'];
