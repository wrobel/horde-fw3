    ...
    if ($type == 'ASCII') {
        ...
        if ($tag == '0001') { //first chunk
            ...
            $result['AFPointUsed']=hexdec(intel2Moto(substr($data,$place,4)));$place+=4;//14
                $afPointUsed = array();
                if ($result['AFPointUsed'] & 0x0001) $afPointUsed[] = _("Right"); //bit 0
                if ($result['AFPointUsed'] & 0x0002) $afPointUsed[] = _("Center"); //bit 1
                if ($result['AFPointUsed'] & 0x0004) $afPointUsed[] = _("Left"); //bit 2
