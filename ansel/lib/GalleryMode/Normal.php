...
class Ansel_GalleryMode_Normal {
    ...
    function getGalleryChildren($perm = PERMS_SHOW, $from = 0, $to = 0)
    {
        ...
        if ($this->hasSubGalleries()) {
            ...
            $num_galleries = $GLOBALS['ansel_storage']->countGalleries(
                Auth::getAuth(), PERMS_SHOW, null, $this->_gallery, false);
    ...
    function countGalleryChildren($perm = PERMS_SHOW, $galleries_only = false)
    {
        ...
        $gCnt = $GLOBALS['ansel_storage']->countGalleries(Auth::getAuth(),
                                                          $perm, null,
                                                          $this->_gallery, false);
    ...
    function moveImagesTo($images, $gallery)
    {
        ...
        } elseif (!$this->_gallery->hasPermission(Auth::getAuth(), PERMS_DELETE)) {
    ...
    function removeImage($image, $isStack)
    {
        ...
        /* Clear any comments */
        if (($GLOBALS['conf']['comments']['allow'] == 'all' || ($GLOBALS['conf']['comments']['allow'] == 'authenticated' && Auth::getAuth())) &&
