 ...
 *
 * $Horde: ansel/lib/Widget/ImageFaces.php,v 1.21.2.5 2009/02/10 01:05:49 mrubinsk Exp $
 *
...
class Ansel_Widget_ImageFaces extends Ansel_Widget {
    ...
    function _getFaceNames()
    {
        ...
        if ($this->_view->gallery->hasPermission(Auth::getAuth(), PERMS_EDIT)) {
            ...
            $html .= '<a id="edit_faces" href="' . Util::addParameter(Horde::applicationUrl('faces/gallery.php'), 'gallery', $this->_view->gallery->id)
                    . '" class="widget">' . $link_text . '</a> | '
                    . Horde::link(Horde::applicationUrl(Util::addParameter('faces/custom.php', array('image' => $this->_view->resource->id, 'url' => $this->_params['selfUrl']))),'', 'widget')
