    ...
    foreach ($info as $i) {
        ...
        if ($i['list']) {
            $s .= "<th class=\"<?php echo (\$sortby == ZOMBIE_SORT_$n) ? 'selected' : 'item' ?>\" width=\"2%\">
                <?php if (\$sortby == ZOMBIE_SORT_$n) echo Horde::link(Horde::applicationUrl(Util::addParameter(\$sortbyurl, 'sortby', ZOMBIE_SORT_$n)), _(\"Change sort direction\"), 'widget') . Horde::img(\$sortdir ? 'za.gif' :  'az.gif', _(\"Change sort direction\"), null, \$registry->get('graphics', 'horde')) ?></a>
                <?php echo Horde::widget(Horde::applicationUrl(Util::addParameter('list.php', 'sortby', ZOMBIE_SORT_$n)), _(\"Sort by User Name\"), 'widget', '', '', _(\"" . $i['desc'] . "\")) ?></a>&nbsp;</th>\n";
        }
    }
...
function t_edit($c)
{
    ...
    foreach ($info as $i) {
        $n = $i['name']; // shortcut
        if (is_datetime($i)) {
            $s .= "        \$zitem['$n'] = Zombie::getDateTime(Util::getFormData('$n'));\n";
        } else if (is_boolean($i)) {
                $s .= "        \$zitem['$n'] = Util::getFormData('$n') ? 1 : 0 ;\n";
            } else {
                $s .= "        \$zitem['$n'] = Util::getFormData('$n');\n";
            }
    }
...
function t_list($c)
{
    ...
    foreach ($info as $i) {
        if ($i['search']) {
            $n = $i['name']; // shortcut
            $s .= "    \$search_$n = (Util::getFormData('search_$n') == 'on');\n";
        }
    }
...
function render_field($field)
{
    ...
    case 'longblob':
    case 'longtext':
        return "nl2br(Text::linkUrls(Text::htmlSpaces(\$zitem['$n']), false, 'text'))";
...
function field_sql2php($field)
{
    ...
    case 'longblob':
    case 'longtext':
        return "String::convertCharset(\$row['".$field["name"]."'], \$this->_params['charset'])";
...
function field_get_quoted($field)
{
    ...
    case 'longblob':
    case 'longtext':
        return "String::convertCharset(\$this->_db->quote(\$zitem['$n']), NLS::getCharset(), \$this->_params['charset'])";
