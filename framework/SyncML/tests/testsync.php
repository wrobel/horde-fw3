...
function testPre($name, $number)
{
    ...
    if (!preg_match('|<SyncHdr>.*?<Source>\s*<LocURI>(.*?)</LocURI>.*?</SyncHdr>|si', $ref0, $m)) {
        echo $ref0;
        Horde::fatal('Unable to find device id', __FILE__, __LINE__);
    }
    $device_id = $m[1];
    ...
    if (preg_match_all('|<Add>.*?<type[^>]*>(.*?)</type>.*?<LocURI[^>]*>(.*?)</LocURI>.*?<data[^>]*>(.*?)</data>.*?</Add|si', $ref1, $m, PREG_SET_ORDER)) {
        foreach ($m as $c) {
            ...
            default:
                Horde::fatal("Unable to find service for contentType=$contentType", __FILE__, __LINE__);
            }
    ...
    if (preg_match_all('|<Replace>.*?<type[^>]*>(.*?)</type>.*?<LocURI[^>]*>(.*?)</LocURI>.*?<data[^>]*>(.*?)</data>.*?</Replace|si', $ref1, $m, PREG_SET_ORDER)) {
        foreach ($m as $c) {
            ...
            if (empty($suid)) {
                Horde::fatal("Unable to find SUID for CUID $locuri for simulating replace", __FILE__, __LINE__);
            }
    ...
    if (preg_match_all('|<Delete>.*?<Target>\s*<LocURI>(.*?)</LocURI>|si', $ref1, $m, PREG_SET_ORDER)) {
        foreach ($m as $d) {
            ...
            if (empty($suid)) {
                Horde::fatal("Unable to find SUID for CUID $locuri for simulating $service delete", __FILE__, __LINE__);
            }
...
function sortChanges($content)
{
    ...
    if (preg_match_all('!<(?:Add|Replace|Delete)>.*?</(?:Add|Replace|Delete)>!si', $content, $ma)) {
        ...
        if (strlen($r) != strlen($bak)) {
            echo "error!\nbefore: $bak\nafter:  $r\n";
            var_dump($m);
            Horde::fatal('failed', __FILE__, __LINE__);
        }
        ...
        return $r;
        //        Horde::fatal('OK', __FILE__, __LINE__);
    }

    return $content;
//    Horde::fatal('not found', __FILE__, __LINE__);
}
