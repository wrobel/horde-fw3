 ...
 * @package Hermes
 *
 * $Horde: hermes/lib/Forms/Search.php,v 1.8.2.1 2009/01/06 15:23:59 jan Exp $
 *
...
class SearchForm extends Horde_Form {
    ...
    function SearchForm(&$vars)
    {
        ...
        if ($GLOBALS['perms']->hasPermission('hermes:review', Auth::getAuth(), PERMS_SHOW)) {
    ...
    function getClientsType()
    {
        ...
        if (is_a($clients, 'PEAR_Error')) {
            ...
            $clients = array_merge(array('' => _("- - None - -")), $clients);
    ...
    function getCostObjectType($vars)
    {
        ...
        foreach ($clients as $client) {
            $criteria = array('user' => Auth::getAuth(),
                              'active' => true,
                              'client_id' => $client);
    ...
    function getSearchCriteria(&$vars)
    {
        ...
        if ($GLOBALS['perms']->hasPermission('hermes:review', Auth::getAuth(), PERMS_SHOW)) {
            ...
            $criteria['employee'] = Auth::getAuth();
        }
